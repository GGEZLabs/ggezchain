...
var (
	...
	vestingAmountVested     = sdk.NewCoin(uggez1Denom, math.NewInt(99900000000))
	vestingAmount           = sdk.NewCoin(uggez1Denom, math.NewInt(350000))
	vestingBalance          = sdk.NewCoins(vestingAmountVested).Add(vestingAmount)
	vestingDelegationAmount = sdk.NewCoin(uggez1Denom, math.NewInt(500000000))
	vestingDelegationFees   = sdk.NewCoin(uggez1Denom, math.NewInt(1))
)
...
func (s *IntegrationTestSuite) testDelayedVestingAccount(api string) {
	...
	s.Run("test delayed vesting genesis account", func() {
		...
		balance, err := getSpecificBalance(api, vestingDelayedAcc.String(), uggez1Denom)
		s.Require().NoError(err)
		s.Require().Equal(vestingBalance.AmountOf(uggez1Denom), balance.Amount)

		// Delegate coins should succeed
		s.execDelegate(chain, valIdx, vestingDelegationAmount.String(), valOpAddr,
			vestingDelayedAcc.String(), ggezHomePath, vestingDelegationFees.String())
...
func (s *IntegrationTestSuite) testContinuousVestingAccount(api string) {
	s.Run("test continuous vesting genesis account", func() {
		...
		//	Check address balance
		balance, err := getSpecificBalance(api, continuousVestingAcc.String(), uggez1Denom)
		s.Require().NoError(err)
		s.Require().Equal(vestingBalance.AmountOf(uggez1Denom), balance.Amount)

		// Delegate coins should succeed
		s.execDelegate(chain, valIdx, vestingDelegationAmount.String(),
			valOpAddr, continuousVestingAcc.String(), ggezHomePath, vestingDelegationFees.String())
...
func (s *IntegrationTestSuite) testPeriodicVestingAccount(api string) {
	s.Run("test periodic vesting genesis account", func() {
		...
		s.execCreatePeriodicVestingAccount(
			chain,
			periodicVestingAddr,
			filepath.Join(ggezHomePath, vestingPeriodFile),
		...
		//	Check address balance
		balance, err := getSpecificBalance(api, periodicVestingAddr, uggez1Denom)
		s.Require().NoError(err)

		expectedBalance := sdk.NewCoin(uggez1Denom, math.NewInt(0))
		for _, period := range acc.VestingPeriods {
			// _, coin := ante.Find(period.Amount, uggez1Denom)
			_, coin := period.Amount.Find(uggez1Denom)
		...
		// Delegate coins should succeed
		s.execDelegate(chain, valIdx, vestingDelegationAmount.String(), valOpAddr,
			periodicVestingAddr, ggezHomePath, vestingDelegationFees.String())
...
func generateVestingPeriod() ([]byte, error) {
	p := vestingPeriod{
		...
		Periods: []period{
			{
				Coins:  "850000000" + uggez1Denom,
				Length: 35,
			},
			{
				Coins:  "2000000000" + uggez1Denom,
				Length: 35,
			},
		},
	}
