...
func (s *IntegrationTestSuite) testStaking() {
	...
	delegation := sdk.NewCoin(uggez1Denom, delegationAmount) // 500 ggez1

	// Alice delegate uggez1 to Validator A
	s.execDelegate(s.chainA, 0, delegation.String(), validatorAddressA, delegatorAddress.String(), ggezHomePath, fees.String())
	...
	redelegation := sdk.NewCoin(uggez1Denom, redelegationAmount) // 250 ggez1

	// Alice re-delegate half of her uggez1 delegation from Validator A to Validator B
	s.execRedelegate(s.chainA, 0, redelegation.String(), validatorAddressA, validatorAddressB, delegatorAddress.String(), ggezHomePath, fees.String())
	...
	s.Require().Eventually(
		func() bool {
			...
			currDelegation = sdk.NewCoin(uggez1Denom, currDelegationAmount)
	...
	// Alice unbonds all her uggez1 delegation from Validator A
	s.execUnbondDelegation(s.chainA, 0, currDelegation.String(), validatorAddressA, delegatorAddress.String(), ggezHomePath, fees.String())
