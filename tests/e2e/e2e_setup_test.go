...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	// 2. Create and initialize ggez validator genesis files (both chains)
...
func (s *IntegrationTestSuite) addGenesisVestingAndJailedAccounts(
	...
	stakingModuleBalances := banktypes.Balance{
		...
		Coins:   sdk.NewCoins(sdk.NewCoin(uggez1Denom, math.NewInt(slashingShares))),
	}
...
func (s *IntegrationTestSuite) initValidatorConfigs(c *chain) {
	for i, val := range c.validators {
		...
		appConfig.MinGasPrices = fmt.Sprintf("%s%s", minGasPrice, uggez1Denom)
...
func (s *IntegrationTestSuite) runValidators(c *chain, portOffset int) {
	s.T().Logf("starting ggez %s validator containers...", c.id)
	...
	for i, val := range c.validators {
		runOpts := &dockertest.RunOptions{
			...
			Mounts: []string{
				fmt.Sprintf("%s/:%s", val.configDir(), ggezHomePath),
			},
			Repository: "ggezlabs/ggezchain",
			Tag:        "latest",
		}
		...
		s.T().Logf("started ggez %s validator container: %s", c.id, resource.Container.ID)
	}
	...
	s.Require().Eventually(
		...
		5*time.Minute,
		time.Second,
		"ggez node failed to produce blocks",
	)
}
...
func (s *IntegrationTestSuite) runIBCRelayer() {
	...
	tmpDir, err := os.MkdirTemp("", "ggez-e2e-testnet-hermes-")
	s.Require().NoError(err)
	s.tmpDirs = append(s.tmpDirs, tmpDir)

	ggezAVal := s.chainA.validators[0]
	ggezBVal := s.chainB.validators[0]

	ggezARly := s.chainA.genesisAccounts[relayerAccountIndexHermes]
	ggezBRly := s.chainB.genesisAccounts[relayerAccountIndexHermes]
	...
	s.hermesResource, err = s.dkrPool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				fmt.Sprintf("GGEZ_A_E2E_CHAIN_ID=%s", s.chainA.id),
				fmt.Sprintf("GGEZ_B_E2E_CHAIN_ID=%s", s.chainB.id),
				fmt.Sprintf("GGEZ_A_E2E_VAL_MNEMONIC=%s", ggezAVal.mnemonic),
				fmt.Sprintf("GGEZ_B_E2E_VAL_MNEMONIC=%s", ggezBVal.mnemonic),
				fmt.Sprintf("GGEZ_A_E2E_RLY_MNEMONIC=%s", ggezARly.mnemonic),
				fmt.Sprintf("GGEZ_B_E2E_RLY_MNEMONIC=%s", ggezBRly.mnemonic),
				fmt.Sprintf("GGEZ_A_E2E_VAL_HOST=%s", s.valResources[s.chainA.id][0].Container.Name[1:]),
				fmt.Sprintf("GGEZ_B_E2E_VAL_HOST=%s", s.valResources[s.chainB.id][0].Container.Name[1:]),
			},
			User: "root",
			Entrypoint: []string{
				"sh",
	...
	// create the client, connection and channel between the two ggez chains
	s.createConnection()
	s.createChannel()
}
		...
		"messages": [
		 {
		  ...
		  "authority": "ggez10d07y265gmmuvt4z0w9aw880jnsr700jm4garg",
		  "plan": {
		   "name": "%s",
		   "height": "%d",
		   "info": "test",
...
func configFile(filename string) string {
	filepath := filepath.Join(ggezConfigPath, filename)
	return filepath
}
