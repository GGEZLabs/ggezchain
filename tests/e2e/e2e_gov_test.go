...
func (s *IntegrationTestSuite) GovCommunityPoolSpend() {
	...
	sendAmount := sdk.NewCoin(uggez1Denom, math.NewInt(10000000)) // 10uggez1
	s.writeGovCommunitySpendProposal(s.chainA, sendAmount, recipient)

	beforeRecipientBalance, err := getSpecificBalance(chainAAPIEndpoint, recipient, uggez1Denom)
	...
	s.Require().Eventually(
		func() bool {
			afterRecipientBalance, err := getSpecificBalance(chainAAPIEndpoint, recipient, uggez1Denom)
			s.Require().NoError(err)
...
// you can use `ggezchaind tx gov draft-proposal` to create a proposal file that you can use
// min initial deposit of 100uggez1 is required in e2e tests, otherwise the proposal would be dropped
