...
func (s *IntegrationTestSuite) testIBCTokenTransfer() {
	s.Run("send_uggez1_to_chainB", func() {
		...
		tokenAmt := 3300000000
		s.sendIBC(s.chainA, 0, sender, recipient, strconv.Itoa(tokenAmt)+uggez1Denom, standardFees.String(), "", false)
...
// 	s.Run("send_successful_multihop_uggez1_to_chainA_from_chainA", func() {
// 		// require the recipient account receives the IBC tokens (IBC packets ACKd)
// 		var (
// 			err error
// 		)
...
// 		var (
// 			beforeSenderuggez1Balance    sdk.Coin
// 			beforeRecipientuggez1Balance sdk.Coin
// 		)

// 		s.Require().Eventually(
// 			func() bool {
// 				beforeSenderuggez1Balance, err = getSpecificBalance(chainAAPIEndpoint, sender, uggez1Denom)
// 				s.Require().NoError(err)

// 				beforeRecipientuggez1Balance, err = getSpecificBalance(chainAAPIEndpoint, recipient, uggez1Denom)
// 				s.Require().NoError(err)

// 				return beforeSenderuggez1Balance.IsValid() && beforeRecipientuggez1Balance.IsValid()
// 			},
// 			1*time.Minute,
// 			5*time.Second,
// 		)
...
// 		s.sendIBC(s.chainA, 0, sender, middlehop, strconv.Itoa(tokenAmt)+uggez1Denom, standardFees.String(), string(memo), false)
...
// 		s.Require().Eventually(
// 			func() bool {
// 				afterSenderuggez1Balance, err := getSpecificBalance(chainAAPIEndpoint, sender, uggez1Denom)
// 				s.Require().NoError(err)

// 				afterRecipientuggez1Balance, err := getSpecificBalance(chainAAPIEndpoint, recipient, uggez1Denom)
// 				s.Require().NoError(err)

// 				decremented := beforeSenderuggez1Balance.Sub(tokenAmount).Sub(standardFees).IsEqual(afterSenderuggez1Balance)
// 				incremented := beforeRecipientuggez1Balance.Add(tokenAmount).IsEqual(afterRecipientuggez1Balance)
...
// 	s.Run("send_failed_multihop_uggez1_to_chainA_from_chainA", func() {
// 		address, _ := s.chainA.validators[0].keyInfo.GetAddress()
// 		sender := address.String()
...
// 				return beforeSenderuggez1Balance.IsValid()
// 			},
// 			1*time.Minute,
// 			5*time.Second,
// 		)
...
// 				returned := beforeSenderuggez1Balance.Sub(tokenAmount).Sub(standardFees).IsEqual(afterSenderuggez1Balance)
...
// 				afterSenderuggez1Balance, err := getSpecificBalance(chainAAPIEndpoint, sender, uggez1Denom)
// 				s.Require().NoError(err)
// 				returned := beforeSenderuggez1Balance.Sub(standardFees).IsEqual(afterSenderuggez1Balance)
// 				return returned
// 			},
// 			5*time.Minute,
// 			10*time.Second,
